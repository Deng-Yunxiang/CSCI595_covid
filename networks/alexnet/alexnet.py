...
class AlexnetFineTuning(LightningModule):
    """
    ...
    def __init__(self, input_shape):
        ...
        # Initializing a pretrained alexnet model.
        self.feature_extractor = models.alexnet(pretrained=False)
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)
        # x = self.classfier(x)
        x = self.feature_extractor.classifier[6](x)
        x = x.squeeze()
        x = self.dropout(x)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=1e-5, weight_decay=1e-2)
        lr_scheduler = {
        'scheduler': torch.optim.lr_scheduler.StepLR(optimizer, step_size=90, 
        gamma=0.8),
        'name': 'learning rate',
         }
